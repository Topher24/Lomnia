...
Character enemy = Character("ENEMY  ", 25,    0, 10, 10, 1.0, 2.0, 0.5, 1.0, false);
bool inCombat = false;
std::clock_t start;
double duration;
...
glm::vec2 moveEnemy()
{
	...
	if (Level[x + 3][z].display == '_' && lastDir != 1) {
		//cout << " 1" << endl;
		movePos = glm::vec2(0.1, 0);
		lastDir = 1;
	}
	else if (Level[x - 3][z].display == '_' && lastDir != 2) {
		movePos = glm::vec2(-0.1, 0);
		//cout << " 2" << endl;
		lastDir = 2;
	}
	else if (Level[x][z + 3].display == '_' && lastDir != 3) {
		movePos = glm::vec2(0, 0.1);
	//	cout << " 3" << endl;
		lastDir = 3;
	}
	else if (Level[x][z - 3].display == '_' && lastDir != 4) {
		movePos = glm::vec2(0, -0.1);
	//	cout << " 4" << endl;
		lastDir = 4;
	}
...
void update(void) {
	...
	{
		playerPos = moveForward(playerPos, r, 0.1f);
		currentAnim = 1;

	}
	else if (keys[SDL_SCANCODE_S]) {
		playerPos = moveForward(playerPos, -r - 180, -0.1f);
		currentAnim = 13;
	...
	int x = enemyPos.x;
	int z = enemyPos.z;
	//cout << Level[x/3][z/3].display << "    " << x << "  " << z << endl;
	enemyPos = glm::vec3(enemyPos.x + enemyMove.x, enemyPos.y, enemyPos.z + enemyMove.y);
	//cout << enemyPos.x << " " << enemyPos.y << " " << enemyPos.z << endl;
	//cout << enemyMove.x << " " << enemyMove.y << endl;
...
void draw(SDL_Window * window) {
	...
	if (camera == 2) {
		at = glm::vec3(playerPos.x, playerPos.y + 1, playerPos.z);
		eye = moveForward(at, r, -5.0f);
		mvStack.top() = glm::lookAt(eye, at, up);
	}
	if (camera == 3){
	at = glm::vec3(enemyPos.x, enemyPos.y + 1, enemyPos.z);
		eye = moveForward(at, r, -5.0f);
		mvStack.top() = glm::lookAt(eye, at, up);
	}
	...
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN - r / 57.5), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	rt3d::setMaterial(shaderProgram, tmpMaterial);
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(playerPos.x + 3, playerPos.y, playerPos.z));
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN), glm::vec3(-1.0f, 0.0f, 0.0f));
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN - r / 57.5), glm::vec3(0.0f, 0.0f, 1.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(scale*0.03, scale*0.03, scale*0.03));
	rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	...
	rt3d::setMaterial(shaderProgram, tmpMaterial);
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(playerPos.x, playerPos.y, playerPos.z + 3));
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN), glm::vec3(-1.0f, 0.0f, 0.0f));
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN - r / 57.5), glm::vec3(0.0f, 0.0f, 1.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(scale*0.03, scale*0.03, scale*0.03));
	rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	...
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN)*ER, glm::vec3(-1.0f, 0.0f, 0.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(scale*0.04, scale*0.04, scale*0.04));
	rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
