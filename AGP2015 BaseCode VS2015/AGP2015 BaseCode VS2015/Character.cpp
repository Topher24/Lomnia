...
void Character::Attack(Character& enemyCharacter)
{
	...
	if (refreshTime <= 0)
	{
		if (queuedAttacks.size() > 0)
		{
			...
			std::advance(it, 0);
			//cout << enemyCharacter.name << " is attacking! Press input: " << it->blockingButton << " to deflect damage!" << endl;
			//it->Attack(*this, enemyCharacter);
			//enemyCharacter.opponentAttack = *it;
...
void Character::BeingAttacked(C_Attack s_attack)
{
	...
	if (player)
		cout << "***************** PRESS " << s_attack.blockingButton << " TO BLOCK ATTACK! *************" << endl;
}

void Character::Damage(int damageValue)
{
	cout << name <<  " damage done!" << damageValue << "health " << health << endl;
...
void Character::Update(float time)
{
	...
	//if(!player)
		//cout << "opponen : " << opponentAttack.attackCompleted << "blocking time: " << opponentAttack.blockingTime << " in combat " << inCombat << " combat name: " << opponentAttack.GetAttackName() << endl;
	...
	if (inCombat)
	{
		if (player)
		{
			if (opponentAttack.blockingTime >= 0 && opponentAttack.attackCompleted == false)
			{
				...
				{
					//cout << " Error is here " << endl;					//what does this mean
					opponentAttack.FailedBlockedAttack(*this, *this);
				}
			}
		} else {
